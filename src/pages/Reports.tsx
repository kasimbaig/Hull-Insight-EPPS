...
const mockDockingReports: DockingReport[] = [
  {
    id: 1,
...
const mockSurveyReports: SurveyReport[] = [
  {
    id: 1,
...
const Reports = () => {
  ...
  const fetchVessels = useCallback(async () => {
    ...
    try {
      ...
      const transformedVessels: Vessel[] = Array.isArray(vesselsData) 
  ...
  const fetchHvacReports = useCallback(async (shipId: number) => {
    if (!shipId) {
      toast({
        title: "Error",
  ...
  const handlePreviewReportWrapper = useCallback((rowData: any) => {
    handlePreviewReport(
      rowData,
  ...
  const handleDownloadReportWrapper = useCallback((rowData: any) => {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 max-w-full overflow-hidden">
      ...
      <SummaryCards
        ...
        onRefreshHvacReports={handleRefreshHvacReports}
        selectedVessel={filters.selectedVessel}
      />
