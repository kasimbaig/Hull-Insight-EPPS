...
export default function SystemsMaster() {
  ...
  const loadSystems = useCallback(async (page = 0, search = '') => {
    setLoading(true);
    try {
      ...
      const response = await getRequest(`master/systems/?${params}`);
      setSystems(response.results);
      setTotalRecords(response.count);
      setCurrentPage(page);
    } catch (error: unknown) {
      console.error('Error loading systems:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to load systems';
      toast({
        title: "Error",
  ...
  const systemFormConfig: FormConfig = {
    title: "System",
    description: "Manage vessel system information",
  ...
  const systemViewConfig = {
    ...
    fields: [
      { key: 'name', label: 'System Name', type: 'text' as const },
      { key: 'code', label: 'System Code', type: 'text' as const },
  ...
  const openForm = (mode: 'create' | 'edit', system?: System) => {
    setFormMode(mode);
    setSelectedSystem(system || null);
    setFormVisible(true);
  };

  const openViewDetails = (system: System) => {
  ...
  const handleSubmit = async (formData: Record<string, unknown>) => {
    ...
    try {
      if (formMode === 'create') {
        ...
        await postRequest('master/systems/', payload);
        toast({
          title: "Success",
          description: "System created successfully",
        });
      } else if (formMode === 'edit' && selectedSystem) {
      ...
      // Reload the data
      await loadSystems(currentPage, globalFilter);
    } catch (error: unknown) {
      console.error('Error saving system:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to save system';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await postRequest('master/systems/', {
        id: selectedSystem.id,
        delete: true
      });
      toast({
      ...
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to delete system';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const actionBodyTemplate = (rowData: System) => {
    return (
  ...
  const statusBodyTemplate = (rowData: System) => {
    return (
  ...
  const handleSearch = async (value: string) => {
    ...
    try {
      await loadSystems(0, value);
    } catch (error) {
      // Error is already handled in loadSystems function
    ...
    <div className="flex flex-wrap gap-4 items-center justify-between p-6 bg-gradient-to-r from-[#8B3A3A]/5 to-[#8B3A3A]/10 rounded-xl border border-[#8B3A3A]/20">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">Systems Master</h2>
            <p className="text-sm text-muted-foreground">Manage vessel systems and components</p>
          </div>
        </div>
    ...
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-xl shadow-lg border border-border overflow-hidden">
        <DataTable
          ...
          onPage={async (e) => {
            try {
              await loadSystems(e.page, globalFilter);
            } catch (error) {
              // Error is already handled in loadSystems function
      ...
      <ViewDetailsForm
        ...
        title={systemViewConfig.title}
        description={systemViewConfig.description}
        data={selectedSystem || {}}
        fields={systemViewConfig.fields}
        sections={systemViewConfig.sections}
      />
