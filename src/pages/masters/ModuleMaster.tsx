...
export default function ModuleMaster() {
  ...
  const loadModules = useCallback(async (page = 0, search = '') => {
    setLoading(true);
    try {
      ...
      const response = await getRequest(`master/modules/?${params}`);
  ...
  const moduleFormConfig: FormConfig = {
    title: "Module",
    ...
    fields: [
      {
        ...
        type: "text",
        required: true,
        placeholder: "e.g., Navigation Module",
  ...
  const moduleViewConfig = {
    ...
    fields: [
      { key: 'name', label: 'Module Name', type: 'text' as const },
      { key: 'code', label: 'Module Code', type: 'text' as const },
  ...
  const openForm = (mode: 'create' | 'edit', module?: Module) => {
    setFormMode(mode);
    setSelectedModule(module || null);
    setFormVisible(true);
  };

  const openViewDetails = (module: Module) => {
  ...
  const handleSubmit = async (formData: Record<string, unknown>) => {
    ...
    try {
      if (formMode === 'create') {
        ...
        await postRequest('master/modules/', payload);
        toast({
          title: "Success",
          description: "Module created successfully",
        });
      } else if (formMode === 'edit' && selectedModule) {
        const payload = {
          name: formData.name as string,
          code: formData.code as string,
      ...
      // Reload the data
      await loadModules(currentPage, globalFilter);
    } catch (error: unknown) {
      console.error('Error saving module:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to save module';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await postRequest('master/modules/', {
        id: selectedModule.id,
        delete: true
      });
      toast({
  ...
  const actionBodyTemplate = (rowData: Module) => {
    return (
  ...
  const statusBodyTemplate = (rowData: Module) => {
    return (
  ...
  const handleSearch = async (value: string) => {
    ...
    try {
      await loadModules(0, value);
    } catch (error) {
      // Error is already handled in loadModules function
    ...
    <div className="flex flex-wrap gap-4 items-center justify-between p-6 bg-gradient-to-r from-[#00809D]/5 to-[#00809D]/10 rounded-xl border border-[#00809D]/20">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">Module Master</h2>
            <p className="text-sm text-muted-foreground">Manage vessel modules and components</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <InputText
              ...
              placeholder="Search modules, names, codes..."
    ...
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-xl shadow-lg border border-border overflow-hidden">
        <DataTable
          ...
          onPage={async (e) => {
            try {
              await loadModules(e.page, globalFilter);
            } catch (error) {
              // Error is already handled in loadModules function
      ...
      <ViewDetailsForm
        ...
        title={moduleViewConfig.title}
        description={moduleViewConfig.description}
        data={selectedModule || {}}
        fields={moduleViewConfig.fields}
        sections={moduleViewConfig.sections}
      />
