...
export default function EquipmentsMaster() {
  ...
  const loadEquipments = useCallback(async (page = 0, search = '') => {
    setLoading(true);
    try {
      ...
      const response = await getRequest(`master/equipments/?${params}`);
      ...
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to load equipments';
      toast({
        title: "Error",
  ...
  const equipmentFormConfig: FormConfig = {
    title: "Equipment",
    description: "Manage vessel equipment information",
  ...
  const equipmentViewConfig = {
    title: "Equipment Details",
    description: "View equipment information",
    fields: [
      { key: 'name', label: 'Equipment Name', type: 'text' as const },
      { key: 'code', label: 'Equipment Code', type: 'text' as const },
  ...
  const openForm = (mode: 'create' | 'edit', equipment?: Equipment) => {
    setFormMode(mode);
    setSelectedEquipment(equipment || null);
    setFormVisible(true);
  };

  const openViewDetails = (equipment: Equipment) => {
  ...
  const handleSubmit = async (formData: Record<string, unknown>) => {
    ...
    try {
      if (formMode === 'create') {
        ...
        await postRequest('master/equipments/', payload);
        toast({
          title: "Success",
          description: "Equipment created successfully",
        });
      } else if (formMode === 'edit' && selectedEquipment) {
      ...
      // Reload the data
      await loadEquipments(currentPage, globalFilter);
    } catch (error: unknown) {
      console.error('Error saving equipment:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to save equipment';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await postRequest('master/equipments/', {
        id: selectedEquipment.id,
        delete: true
      });
      toast({
      ...
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to delete equipment';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const actionBodyTemplate = (rowData: Equipment) => {
    return (
  ...
  const handleSearch = async (value: string) => {
    ...
    try {
      await loadEquipments(0, value);
    } catch (error) {
      // Error is already handled in loadEquipments function
    ...
    <div className="flex flex-wrap gap-4 items-center justify-between p-6 bg-gradient-to-r from-[#00809D]/5 to-[#00809D]/10 rounded-xl border border-[#00809D]/20">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">Equipments Master</h2>
            <p className="text-sm text-muted-foreground">Manage vessel equipment and machinery</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <InputText
              ...
              placeholder="Search equipments, names, codes..."
    ...
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-xl shadow-lg border border-border overflow-hidden">
        <DataTable
          ...
          onPage={async (e) => {
            try {
              await loadEquipments(e.page, globalFilter);
            } catch (error) {
              // Error is already handled in loadEquipments function
      ...
      <ViewDetailsForm
        ...
        title={equipmentViewConfig.title}
        description={equipmentViewConfig.description}
        data={selectedEquipment || {}}
        fields={equipmentViewConfig.fields}
        sections={equipmentViewConfig.sections}
      />
