...
export default function SeveritiesMaster() {
  ...
  const loadSeverities = useCallback(async (page = 0, search = '') => {
    setLoading(true);
    try {
      ...
      const response = await getRequest(`master/severities/?${params}`);
      setSeverities(response.results);
      setTotalRecords(response.count);
      setCurrentPage(page);
    } catch (error: unknown) {
      console.error('Error loading severities:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to load severities';
      toast({
        title: "Error",
  ...
  // Load severities on component mount
  useEffect(() => {
    loadSeverities();
  }, [loadSeverities]);

  const severityFormConfig: FormConfig = {
    title: "Severity",
    description: "Manage damage severity levels",
  ...
  const severityViewConfig = {
    title: "Severity Details",
    description: "View severity level information",
    fields: [
      { key: 'name', label: 'Severity Name', type: 'text' as const },
      { key: 'code', label: 'Severity Code', type: 'text' as const },
  ...
  const openForm = (mode: 'create' | 'edit', severity?: Severity) => {
    setFormMode(mode);
    setSelectedSeverity(severity || null);
    setFormVisible(true);
  };

  const openViewDetails = (severity: Severity) => {
    setSelectedSeverity(severity);
    setViewDetailsVisible(true);
  };

  const openDeleteDialog = (severity: Severity) => {
    setSelectedSeverity(severity);
  ...
  const handleSubmit = async (formData: Record<string, unknown>) => {
    ...
    try {
      if (formMode === 'create') {
        ...
        await postRequest('master/severities/', payload);
        toast({
          title: "Success",
          description: "Severity created successfully",
        });
      } else if (formMode === 'edit' && selectedSeverity) {
      ...
      // Reload the data
      await loadSeverities(currentPage, globalFilter);
    } catch (error: unknown) {
      console.error('Error saving severity:', error);
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to save severity';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await postRequest('master/severities/', {
        id: selectedSeverity.id,
        delete: true
      });
      toast({
      ...
      const apiError = error as ApiError;
      const errorMessage = apiError?.response?.data?.message || apiError?.message || 'Failed to delete severity';
      toast({
        title: "Error",
        description: errorMessage,
        variant: "destructive",
  ...
  const actionBodyTemplate = (rowData: Severity) => {
    return (
  ...
  const statusBodyTemplate = (rowData: Severity) => {
    return (
  ...
  const handleSearch = async (value: string) => {
    ...
    try {
      await loadSeverities(0, value);
    } catch (error) {
      // Error is already handled in loadSeverities function
    ...
    <div className="flex flex-wrap gap-4 items-center justify-between p-6 bg-gradient-to-r from-[#8B3A3A]/5 to-[#8B3A3A]/10 rounded-xl border border-[#8B3A3A]/20">
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-foreground">Severities Master</h2>
            <p className="text-sm text-muted-foreground">Manage damage severity classifications</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <InputText
              ...
              placeholder="Search severities, names, codes..."
    ...
    <div className="p-6 space-y-6">
      <div className="bg-white rounded-xl shadow-lg border border-border overflow-hidden">
        <DataTable
          ...
          onPage={async (e) => {
            try {
              await loadSeverities(e.page, globalFilter);
            } catch (error) {
              // Error is already handled in loadSeverities function
      ...
      <ViewDetailsForm
        ...
        title={severityViewConfig.title}
        description={severityViewConfig.description}
        data={selectedSeverity || {}}
        fields={severityViewConfig.fields}
        sections={severityViewConfig.sections}
      />
      ...
      <DeleteConfirmationDialog
        ...
        message="Are you sure you want to delete this severity? This action cannot be undone."
