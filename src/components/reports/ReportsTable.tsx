...
import { ReportData, ReportFilters, LoadingStates } from "@/types/reports";

interface ReportsTableProps {
  data: ReportData;
  filters: ReportFilters;
  loadingStates: LoadingStates;
  onPreviewReport: (rowData: any) => void;
  onDownloadReport: (rowData: any) => void;
}
...
export const ReportsTable = ({ 
  ...
  const statusBodyTemplate = useCallback((rowData: any) => {
    const statusConfig = {
      ...
      "Overdue": { variant: "secondary" as const, color: "text-red-600" },
      "Draft": { variant: "outline" as const, color: "text-gray-600" },
      "Command Review": { variant: "secondary" as const, color: "text-blue-600" },
      "IHQ Review": { variant: "secondary" as const, color: "text-orange-600" },
      "Approved": { variant: "secondary" as const, color: "text-green-600" },
      "Archived": { variant: "outline" as const, color: "text-gray-500" }
    };
    
    const config = statusConfig[rowData.status as keyof typeof statusConfig] || { variant: "outline" as const, color: "text-gray-600" };
    
    return (
      <Badge variant={config.variant} className={`${config.color} ${config.bgColor || ''} flex items-center gap-1`}>
        {rowData.status}
      </Badge>
    );
  }, []);

  const dateBodyTemplate = useCallback((rowData: any) => {
    const date = rowData.trial_date || rowData.created_date || rowData.survey_date || rowData.last_updated;
    return date ? new Date(date).toLocaleDateString() : "N/A";
  }, []);

  const occasionBodyTemplate = useCallback((rowData: any) => {
  ...
  const measurementsBodyTemplate = useCallback((rowData: any) => {
    if (rowData.report_type === "HVAC Trial") {
      return (
        <div className="space-y-1">
  ...
  const actionsBodyTemplate = useCallback((rowData: any) => {
