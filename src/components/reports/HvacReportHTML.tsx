...
import { HvacTrial, AirFlowMeasurement, MachineryAirFlowMeasurement } from '@/types/reports';
import { ReportContainer, ReportHeader } from './ReportStyles';

interface HvacReportHTMLProps {
  trial: HvacTrial;
  onClose?: () => void;
}

const HvacReportHTML: React.FC<HvacReportHTMLProps> = ({ trial, onClose }) => {
  ...
  const groupedAirFlowMeasurements = trial.airflow_measurements.reduce((acc, measurement) => {
    ...
    return acc;
  }, {} as Record<string, AirFlowMeasurement[]>);
  ...
  const groupedMachineryMeasurements = trial.machinery_airflow_measurements.reduce((acc, measurement) => {
    ...
    acc[key].push(measurement);
    return acc;
  }, {} as Record<string, MachineryAirFlowMeasurement[]>);
                  ...
                  {Object.entries(groupedAirFlowMeasurements).map(([compartment, measurements], compartmentIndex) => 
                    measurements.map((measurement, measurementIndex) => (
                      <tr key={`${compartment}-${measurementIndex}`} className={measurement.remarks === 'UNSAT' || measurement.observations === 'Sub-optimal air flow' ? 'bg-red-50' : ''}>
                        ...
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.duct_area}</td>
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.air_flow}</td>
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.flow_rate_at_duct}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          <div className="p-6">
            ...
            <div className="w-full table-container">
              <table className="w-full text-xs" style={{ border: '1px solid #d1d5db' }}>
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Ser No</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Served By Blower/ Fan Supply/ Fan Exhaust/ MCS/ MCE/ MCR/ MS/ ME</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Compartment Name</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>No of Ducts</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Duct Area (m²)</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Air Flow (m/s)</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Flow Rate at Duct (m³/hr)</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" colSpan={2}>Total Air Flow Rate in Compartment (m³/hr)</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider style={{ borderRight: '1px solid #d1d5db' }}" rowSpan={2}>Observations</th>
                    <th className="px-1 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider" rowSpan={2}>Remarks</th>
                  </tr>
                  <tr>
                    <th className="px-1 py-1 bg-gray-100 text-xs" style={{ borderRight: '1px solid #d1d5db' }}>Design Value</th>
                  ...
                  {Object.entries(groupedMachineryMeasurements).map(([servedBy, measurements], servedByIndex) => 
                    measurements.map((measurement, measurementIndex) => (
                      <tr key={`${servedBy}-${measurementIndex}`} className={measurement.remarks === 'UNSAT' || measurement.observations === 'Sub-optimal flow' ? 'bg-red-50' : ''}>
                        {measurementIndex === 0 && (
                          <>
                            <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} align-top text-xs" rowSpan={measurements.length}>{servedByIndex + 1}.</td>
                            <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} align-top text-xs" rowSpan={measurements.length}>{servedBy}</td>
                            <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} align-top text-xs" rowSpan={measurements.length}>{measurement.design_air_flow_rate}</td>
                            <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} align-top text-xs" rowSpan={measurements.length}>{measurement.measured_air_flow_rate}</td>
                            <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} align-top text-xs" rowSpan={measurements.length}>{measurement.observations || 'Nil'}</td>
                            <td className="px-1 py-1 align-top font-semibold text-xs" rowSpan={measurements.length}>
                        ...
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.compartment_name_display}</td>
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.no_of_ducts}</td>
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.duct_area}</td>
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.air_flow}</td>
                        <td className="px-1 py-1 style={{ borderRight: '1px solid #d1d5db' }} text-xs">{measurement.flow_rate_at_duct}</td>
                      </tr>
