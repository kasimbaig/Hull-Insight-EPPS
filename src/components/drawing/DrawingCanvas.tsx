...
import { Trash2, Upload, ZoomIn, ZoomOut, Hand } from "lucide-react";
...
export default function Drawing() {
  ...
  const [currentTool, setCurrentTool] = useState<ShapeType | "hand" | null>(
    "rect"
  );
  ...
  useEffect(() => {
    try {
      ...
      if (savedState) {
        const { shapes: savedShapes, backgroundImage: savedImage } =
      ...
      switch (currentTool) {
        case "rect":
          newShape = {
            ...defaultProps,
            type: "rect",
            x: svgPoint.x - defaultWidth / 2,
            y: svgPoint.y - defaultHeight / 2,
            width: defaultWidth,
        ...
        case "triangle":
          newShape = {
            ...
            height: defaultHeight,
            r: 0,
            metadata: { ...defaultProps.metadata, label: "New Triangle" },
          };
          break;
        case "star":
          newShape = {
            ...
            height: defaultHeight,
            r: 0,
            metadata: { ...defaultProps.metadata, label: "New Star" },
          };
          break;
      }
  ...
  return (
    <div className="flex h-screen w-full">
      {/* Left Toolbar */}
      {/* Left Toolbar */}
      <div className="w-20 p-4 border-r bg-gray-50 space-y-4 rounded-md flex flex-col items-center">
        <h2 className="font-bold mb-2 text-xl">Tools</h2>
        ...
        >
          ...
          {shapes.map((shape) => {
            ...
            switch (shape.type) {
              ...
              case "triangle":
                const triPath = `M${shape.x},${shape.y - shape.height / 2} L${
                  shape.x - shape.width / 2
                },${shape.y + shape.height / 2} L${shape.x + shape.width / 2},${
                  shape.y + shape.height / 2
                } Z`;
                return (
                  <g key={shape.id}>
              ...
              case "star":
                const starPath = `M ${shape.x} ${
                  shape.y - shape.height / 2
                } L ${shape.x + shape.width * 0.1} ${
                  shape.y - shape.height * 0.15
                } L ${shape.x + shape.width / 2} ${
                  shape.y - shape.height * 0.15
                } L ${shape.x + shape.width * 0.2} ${
                  shape.y + shape.height * 0.05
                } L ${shape.x + shape.width * 0.3} ${
                  shape.y + shape.height * 0.4
                } L ${shape.x} ${shape.y + shape.height * 0.1} L ${
                  shape.x - shape.width * 0.3
                } ${shape.y + shape.height * 0.4} L ${
                  shape.x - shape.width * 0.2
                } ${shape.y + shape.height * 0.05} L ${
                  shape.x - shape.width / 2
                } ${shape.y - shape.height * 0.15} L ${
                  shape.x - shape.width * 0.1
                } ${shape.y - shape.height * 0.15} Z`;
                return (
                  <g key={shape.id}>
                    <path
                      data-id={shape.id}
